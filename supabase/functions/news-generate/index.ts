...
Deno.serve(async (req: Request) => {
  ...
  if (!newsletterOnly) {
    // Production news is generated by the RSS-grounded GitHub workflow.
    // This endpoint previously generated "current news" without any sources,
    // which could fabricate stories and misclassify them.
    return Response.json({
      error: "Ungrounded generation is disabled",
      note: "Run the Daily AI News Generation workflow so articles pass source and category validation.",
  ...
  const newsletterArticles = articles.filter((a) =>
    a.category !== "breaking"
    && CATEGORIES.some((category) => category.key === a.category)
    && Boolean(a.translations?.en?.title && a.translations?.en?.description)
  );
