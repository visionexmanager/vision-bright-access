...
### What it does not prove — and I will not invent

**CPU cores, CPU model, RAM, free disk, GPU presence and network bandwidth are not
determinable from this repository.** There is no infrastructure manifest recording
them and I have no shell on that host. The `infra/k8s` and Helm material describes a
Kubernetes topology that the actual `deploy.yml` does not use, so it cannot be read as
evidence of the real machine.

The only defensible inference: the box runs a full Vite production build of this app,
which needs roughly **2-4 GB RAM**, so **≥4 GB total** is a safe floor. That is a floor,
not a spec.

**Every sizing recommendation below is conditional on the answer.** Run this and the
audit becomes precise:

```bash
nproc && free -h && df -h / && lscpu | head -20 && (lspci | grep -i -E 'vga|3d|nvidia' || echo 'no discrete GPU') && docker info 2>/dev/null | head -5
```

Per your instruction, **no GPU is assumed anywhere in this document.**
...
Assuming no GPU today: **RAM, to 16 GB**, then cores. It unlocks Tesseract +
...
1. **Server specification unknown.** Section 2. Everything is conditional until the
   one-line command is run.
