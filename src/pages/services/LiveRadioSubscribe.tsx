...
import { ArrowLeft, ArrowRight, Check, Radio, Zap, Crown } from "lucide-react";
...
export default function LiveRadioSubscribe() {
  ...
  const planBadge: Record<string, string | null> = {
    Daily:   null,
    Weekly:  null,
    Monthly: t("liveRadio.mostPopular"),
    Yearly:  t("liveRadio.bestValue"),
  };
      ...
      <div className="max-w-5xl mx-auto px-4 py-12 space-y-10" dir={dir}>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <h1 className="text-3xl font-bold text-foreground">{t("liveRadio.subscribeTitle")}</h1>
            </div>
            <p className="text-muted-foreground">{t("liveRadio.subscribeSubtitle")}</p>
          </div>
        ...
        {isSubscribed && (
          <div className="rounded-xl border border-orange-500/30 bg-orange-500/5 p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <p className="font-semibold text-orange-600">{t("liveRadio.hasActiveSubscription")}</p>
              <p className="text-sm text-muted-foreground mt-1">
                {t("liveRadio.expiresIn").replace("{n}", `${daysRemaining} ${daysRemaining === 1 ? t("liveRadio.day") : t("liveRadio.days")}`)}
              </p>
            </div>
            <Button asChild variant="outline" className="border-orange-500/40 text-orange-600 hover:bg-orange-500/10">
              <Link to="/services/live-radio">{t("liveRadio.backToListen")}</Link>
            </Button>
          </div>
        )}
            ...
            return (
              ...
              >
                  ...
                  <div className="mt-2">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      {plan.duration_days === 1
                        ? t("liveRadio.planOneDay")
                        : t("liveRadio.planNDays").replace("{n}", String(plan.duration_days))}
                    </p>
                  </div>
                </CardHeader>
                ...
                <CardContent className="pt-0 space-y-4">
                  ...
                  >
                    {loadingId === plan.id
                      ? t("liveRadio.subscribingNow")
                      : isSubscribed
                      ? t("liveRadio.alreadySubscribed")
                      : !enough
                      ? t("liveRadio.insufficientBalance")
                      : t("liveRadio.doSubscribeNow")}
                  </Button>

                  {!enough && !isSubscribed && (
                    <p className="text-xs text-center text-destructive">
                      {t("liveRadio.needMoreVX").replace("{n}", (plan.vx_price - balance).toLocaleString())}
                    </p>
                  )}
                </CardContent>
              </Card>
