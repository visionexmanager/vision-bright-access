...
export default function AdminModeration() {
  ...
  return (
    <Layout><section className="mx-auto max-w-6xl px-4 py-10"><div className="mb-6 flex items-center gap-3"><Button asChild variant="ghost" size="icon"><Link to="/admin" aria-label="Back to admin"><ArrowLeft className="h-5 w-5" aria-hidden="true" /></Link></Button><h1 className="text-3xl font-bold">{t("admin.moderation.title")}</h1>{pending > 0 && <Badge className="bg-yellow-500">{t("admin.moderation.pendingBadge").replace("{count}", String(pending))}</Badge>}</div>
