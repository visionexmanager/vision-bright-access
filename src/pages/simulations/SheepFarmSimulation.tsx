  ...
  const simulateMonth = () => {
    ...
    const interval = setInterval(() => {
      ...
      if (step >= total) {
        ...
        if (deaths > 0) toast.error(t("sim.sheepFarm.error.deaths").replace("{count}", String(deaths)));
        if (monthWool > 0) toast.success(t("sim.sheepFarm.success.shearing").replace("{wool}", String(monthWool)));
        else toast.success(t("sim.sheepFarm.success.monthComplete").replace("{month}", String(month)).replace("{health}", String(Math.round(newHealth))));
    ...
    return (
      <Card className="max-w-lg mx-auto animate-in fade-in">
        <CardContent className="p-8 text-center space-y-4">
          ...
          <h2 className="text-2xl font-bold">{t("sim.sheepFarm.report.title")}</h2>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="rounded-xl bg-primary/10 p-3"><p className="text-2xl font-bold text-primary">{woolHarvest}kg</p><p className="text-xs text-muted-foreground">{t("sim.sheepFarm.report.wool")}</p></div>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold">🐑 {started ? t("sim.sheepFarm.month").replace("{month}", String(month)).replace("{total}", String(totalMonths)) : t("sim.sheepFarm.setup")}</h2>
        {started && <Badge variant="secondary" role="status" aria-live="polite">{t("sim.sheepFarm.stats").replace("{alive}", String(alive)).replace("{health}", String(health))}</Badge>}
      </div>
      ...
      {!started && (
        <Card>
          <CardContent className="p-4 space-y-4">
            <h3 className="font-bold text-sm">{t("sim.sheepFarm.setupTitle")}</h3>
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">{t("sim.sheepFarm.breed")}</label>
            ...
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">{t("sim.sheepFarm.flock").replace("{count}", String(flockSize))}</label>
              <Slider value={[flockSize]} onValueChange={([v]) => setFlockSize(v)} min={10} max={100} step={5} />
            </div>
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">{t("sim.sheepFarm.grazing")}</label>
            ...
            <div className="p-3 rounded-lg bg-muted/50 text-xs">
              <div className="flex justify-between font-bold"><span>{t("sim.sheepFarm.costs.setupCost")}</span><span>${setupCost}</span></div>
            </div>
            <Button onClick={startFarm} className="w-full" aria-label="Start Farm">{t("sim.sheepFarm.btn.startFarm")}</Button>
          </CardContent>
        </Card>
      )}
        ...
        <Card>
          <CardContent className="p-4 space-y-4">
            <div className="grid grid-cols-3 gap-2 text-center">
              ...
              <div><p className="text-xs text-muted-foreground">{t("sim.sheepFarm.report.wool")}</p><p className="font-bold">{woolHarvest}kg</p></div>
              <div><p className="text-xs text-muted-foreground">Health</p><p className="font-bold">{health}%</p></div>
            </div>
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">{t("sim.sheepFarm.waterAccess").replace("{level}", String(waterAccess))}</label>
              <Slider value={[waterAccess]} onValueChange={([v]) => setWaterAccess(v)} min={1} max={5} step={1} />
            </div>
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">{t("sim.sheepFarm.shearingFreq").replace("{freq}", String(shearingFreq))}</label>
              <Slider value={[shearingFreq]} onValueChange={([v]) => setShearingFreq(v)} min={1} max={4} step={1} />
            </div>
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">{t("sim.sheepFarm.vaccineBudget").replace("{budget}", String(vaccineBudget))}</label>
              <Slider value={[vaccineBudget]} onValueChange={([v]) => setVaccineBudget(v)} min={0} max={100} step={10} />
            </div>
            <Button onClick={simulateMonth} className="w-full" aria-label={t("sim.sheepFarm.btn.simulateMonth").replace("{month}", String(month))}>{t("sim.sheepFarm.btn.simulateMonth").replace("{month}", String(month))}</Button>
          </CardContent>
        </Card>
      )}
      ...
      {log.length > 0 && (
        <Card>
          <CardContent className="p-3">
            <h3 className="font-bold text-xs mb-2">{t("sim.sheepFarm.history.title")}</h3>
            {log.map((l) => (
