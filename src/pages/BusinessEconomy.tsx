...
export default function BusinessEconomy() {
  ...
  const exportPDF = useCallback(() => {
    ...
    doc.text("Visionex Economy", isRtl ? 195 : 15, 25, { align: isRtl ? "right" : "left" });
    
    doc.setFontSize(14);
    doc.setTextColor(60, 60, 60);
    doc.text(t("econ.feasibilityReport"), isRtl ? 195 : 15, 38, { align: isRtl ? "right" : "left" });
    ...
    addRow(t("econ.projectName"), projectName || "—");
    addRow(t("econ.sector"), sector || "—");
    addRow(t("econ.initialBudget"), `$${parseFloat(budget).toLocaleString()}`);
    addRow(t("econ.monthlyCost"), `$${parseFloat(monthlyCost).toLocaleString()}`);
    addRow(t("econ.monthlyRevenue"), `$${parseFloat(monthlyRevenue).toLocaleString()}`);
    ...
    doc.setFontSize(16);
    doc.setTextColor(16, 185, 129);
    doc.text(t("econ.results"), x, y, { align });
    y += 14;

    addRow(t("econ.roi"), `${result.roi.toFixed(1)}%`);
    addRow(t("econ.breakEven"), `${result.breakEven > 100 ? "∞" : result.breakEven} ${t("econ.months")}`);
    addRow(t("econ.annualProfit"), `$${result.netProfit.toLocaleString()}`);
    addRow(t("econ.riskLevel"), result.riskLevel);
    addRow(t("econ.recommendation"), result.recommendation);

    // Footer
    doc.setFontSize(9);
    doc.setTextColor(160, 160, 160);
    doc.text(`Generated by Visionex Economy — ${new Date().toLocaleDateString()}`, 105, 285, { align: "center" });

    doc.save(`feasibility-${projectName || "report"}.pdf`);
    toast.success(t("econ.pdfExported"));
  }, [result, projectName, sector, budget, monthlyCost, monthlyRevenue, t, lang]);
