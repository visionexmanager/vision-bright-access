...
function detectType(url: string): UrlType {
  ...
  if (url.match(/\.(mp3|aac|ogg|opus|flac|wav)(\?|$)/i)
    ...
    || url.includes("bbcmedia") || url.includes("zenapi")) return "audio";
  return "external";
}
...
// ── YouTube iframe ─────────────────────────────────────────────
function YouTubePlayer({ url, name }: { url: string; name: string }) {
  ...
  return (
    <div className="relative w-full aspect-video bg-black rounded-xl overflow-hidden">
      ...
      />
      <div className="absolute bottom-2 left-2 flex items-center gap-1.5 rounded-full bg-black/60 px-2 py-0.5 text-[10px] text-white/70 pointer-events-none select-none">
...
// ── HLS video ──────────────────────────────────────────────────
function HLSPlayer({ url, name, logo, onError }: { url: string; name: string; logo?: string | null; onError?: () => void }) {
    ...
    (async () => {
      try {
        if (video.canPlayType("application/vnd.apple.mpegurl")) {
          ...
          video.onerror = () => err("تعذّر تشغيل البث");
        } else {
          const Hls = await loadHls();
          if (!Hls.isSupported()) { err("المتصفح لا يدعم HLS"); return; }
          const hls = new Hls({ enableWorker: true });
          hlsRef.current = hls;
          hls.loadSource(url);
          hls.attachMedia(video);
          hls.on(Hls.Events.MANIFEST_PARSED, () => { video.play().catch(() => {}); setLoading(false); setPlaying(true); });
          hls.on(Hls.Events.ERROR, (_, d) => { if (d.fatal) err("تعذّر تشغيل البث"); });
        }
      } catch { err("تعذّر تحميل المشغّل"); }
    })();
    ...
    >
      ...
      {loading && !errMsg && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-black/80 z-10">
          ...
          <p className="text-sm text-white/60">جاري تحميل البث…</p>
        </div>
      )}
      {errMsg && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-black/90 z-10">
          ...
          <Button size="sm" variant="outline" className="text-white border-white/20 hover:bg-white/10"
            ...
            <RotateCcw className="w-3.5 h-3.5 me-1.5" /> إعادة المحاولة
          </Button>
        </div>
      )}
      ...
      {!loading && !errMsg && (
        <div className={cn(
          ...
          {/* Bottom: controls */}
          <div className="flex items-center gap-2">
            <button onClick={() => { const v = videoRef.current; if (!v) return; playing ? v.pause() : v.play(); setPlaying(!playing); }}
      ...
      <div className="absolute bottom-2 left-2 flex items-center gap-1.5 rounded-full bg-black/60 px-2 py-0.5 text-[10px] text-white/70 pointer-events-none select-none z-20">
        <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" />
        بث مباشر رسمي
      </div>
    </div>
...
// ── Audio player (radio) ───────────────────────────────────────
function AudioPlayer({ url, name, logo, onError }: { url: string; name: string; logo?: string | null; onError?: () => void }) {
    ...
    if (isHls) {
      (async () => {
        try {
          ...
          hls.on(Hls.Events.MANIFEST_PARSED, () => { audio.play().then(() => { setLoading(false); setPlaying(true); }).catch(() => { setLoading(false); }); });
          hls.on(Hls.Events.ERROR, (_, d) => { if (d.fatal) err("تعذّر تحميل البث"); });
        } catch { err("تعذّر تحميل المشغّل"); }
      })();
    } else {
      audio.src = url;
      audio.oncanplay = () => { audio.play().then(() => { setLoading(false); setPlaying(true); }).catch(() => setLoading(false)); };
      audio.onerror = () => err("تعذّر الاتصال بمحطة البث");
    }
    ...
    <div className="rounded-2xl border bg-gradient-to-br from-orange-950/60 to-slate-900 p-6 space-y-5">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-1">
            ...
            <span className="text-sm text-white/60">{playing ? "يبث الآن" : loading ? "جارٍ التحميل…" : "متوقف"}</span>
          </div>
        </div>
      </div>
      ...
      {errMsg && (
        <div className="flex items-center gap-2 text-red-400 text-sm">
          ...
          <span>{errMsg}</span>
          <Button size="sm" variant="ghost" className="ms-auto text-white/60 hover:text-white" onClick={() => { setErrMsg(null); setLoading(true); }}>
      ...
      <div className="flex items-center gap-3">
        <button onClick={togglePlay} disabled={loading && !errMsg}
          className={cn(
            "flex h-12 w-12 items-center justify-center rounded-full transition-all flex-shrink-0",
            loading && !errMsg ? "bg-white/10 text-white/30 cursor-not-allowed" : "bg-orange-500 hover:bg-orange-400 text-white shadow-lg shadow-orange-500/30"
          )}>
          {loading && !errMsg
        ...
        <input type="range" min={0} max={100} value={muted ? 0 : volume}
          onChange={(e) => { const v = Number(e.target.value); setVolume(v); if (audioRef.current) { audioRef.current.volume = v / 100; setMuted(v === 0); } }}
      ...
      <p className="text-center text-[10px] text-white/30 flex items-center justify-center gap-1">
        ...
        البث مقدَّم من المصدر الرسمي للمحطة
      </p>
    </div>
  );
}
...
function ExternalPlayer({ url, name, isTV }: { url: string; name: string; isTV?: boolean }) {
  return (
    <div className={cn(
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">البث متاح على الموقع الرسمي</p>
      </div>
      ...
      <p className="text-[10px] text-muted-foreground/60">سيُفتح في تبويب جديد · محتوى مرخَّص من المصدر</p>
    </div>
  );
}
...
export function OfficialStreamPlayer({ url, name, logo, isTV = false, onError }: Props) {
  if (!url) {
    return (
      <div className={cn("rounded-xl border flex items-center justify-center text-muted-foreground", isTV ? "aspect-video bg-muted" : "p-12 bg-card")}>
        <div className="flex flex-col items-center gap-2">
          {isTV ? <Tv className="w-14 h-14 opacity-20" /> : <Radio className="w-12 h-12 opacity-20" />}
          <p className="text-sm">لا يوجد مصدر بث</p>
        </div>
      </div>
    );
  }
  ...
  if (type === "youtube") return <YouTubePlayer url={url} name={name} />;
  if (type === "hls")     return <HLSPlayer url={url} name={name} logo={logo} onError={onError} />;
  if (type === "audio")   return <AudioPlayer url={url} name={name} logo={logo} onError={onError} />;
  return <ExternalPlayer url={url} name={name} isTV={isTV} />;
}
