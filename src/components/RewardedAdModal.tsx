...
// ─── Simulated ad (countdown) used when GAM is not configured ────────────────
      ...
      <div className="relative w-full max-w-sm rounded-2xl border border-border bg-card shadow-2xl p-8 text-center space-y-5">
        {!granted ? (
          <>
            ...
            <p className="text-xs text-muted-foreground">{t("dash.adWatching")}</p>
            <div className="space-y-2">
              <Progress value={progress} className="h-3" />
              <p className="text-2xl font-bold tabular-nums text-amber-500">{t("dash.adSecondsLeft").replace("{s}", String(secondsLeft))}</p>
            </div>
            <p className="text-xs text-muted-foreground">
              +{VX_REWARD} VX {t("dash.adWatched")?.replace("{pts}", "") || "reward on completion"}
            </p>
          </>
        ) : (
          <>
...
// ─── Public export: picks real or simulated automatically ─────────────────────
