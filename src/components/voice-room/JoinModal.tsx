  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div
        ...
        aria-describedby="voice-room-join-desc"
        className="mx-4 w-full max-w-sm rounded-2xl border bg-card p-6 shadow-2xl space-y-5"
      >
        <div className="text-center space-y-1">
          <h2 id="voice-room-join-title" className="text-xl font-bold">{roomName}</h2>
          <p id="voice-room-join-desc" className="text-sm text-muted-foreground">{t("vroom.joinPrompt")}</p>
        </div>

        {hasPassword && (
          <div className="space-y-2">
            <label htmlFor="voice-room-password" className="flex items-center gap-2 text-sm font-medium">
              <Lock className="h-4 w-4 text-muted-foreground" aria-hidden="true" />
              {t("vroom.roomPassword")}
            </label>
            <Input
              id="voice-room-password"
              type="password"
              ...
              aria-invalid={!!error}
              aria-describedby={error ? "voice-room-password-error" : undefined}
              autoFocus
            />
            {error && <p id="voice-room-password-error" className="text-xs text-destructive">{error}</p>}
          </div>
        )}
