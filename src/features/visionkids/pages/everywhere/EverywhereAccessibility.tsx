...
import { Sun, Moon, Eye, Type, Wand2, ScanEye, Keyboard, Mic, Volume2 } from "lucide-react";
...
const SUPPORT = [
  ...
  { icon: Mic, key: "voice" },
  { icon: Volume2, key: "audioGuidance" },
];

/** Accessibility works on every platform — including fully offline for
 *  screen-reader + keyboard users, which is the core promise of Phase 18. */
export default function EverywhereAccessibility() {
  const { t } = useLanguage();
  const { theme, setTheme } = useThemeToggle();
  const [, force] = useState(0);
  const bump = () => force((v) => v + 1);

  useDocumentHead({ title: `${t("kids.everywhere.nav.accessibility")} — VisionKids`, description: t("kids.everywhere.accessibility.subtitle"), canonicalPath: "/kids/everywhere/accessibility" });
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
      <EverywhereHeader emoji="♿" title={t("kids.everywhere.nav.accessibility")} subtitle={t("kids.everywhere.accessibility.subtitle")} />
      <div className="mt-4 rounded-2xl border-2 border-kids-primary/30 bg-kids-primary/5 p-4"><p className="text-sm font-semibold">🔊 {t("kids.everywhere.accessibility.offlinePromise")}</p></div>
      ...
      <div className="mt-5 flex flex-col gap-5">
        ...
        <section className="rounded-2xl border-2 border-border bg-card p-5">
          ...
          <div className="mt-3 flex items-center justify-between gap-4">
            <Label htmlFor="everywhere-reduce-motion" className="text-sm font-medium">{t("kids.settings.reduceMotion")}</Label>
            <Switch id="everywhere-reduce-motion" checked={reduceMotion} onCheckedChange={(c) => { setKidsReduceMotion(c); bump(); }} />
          </div>
        </section>
        <section className="rounded-2xl border-2 border-border bg-card p-5">
          <h2 className="font-heading text-lg font-bold">{t("kids.everywhere.accessibility.supportTitle")}</h2>
          <ul className="mt-3 flex flex-col gap-3">
            {SUPPORT.map(({ icon: Icon, key }) => (
              <li key={key} className="flex items-start gap-3">
                <Icon className="mt-0.5 h-5 w-5 shrink-0 text-kids-primary" aria-hidden="true" />
                <div>
                  <p className="text-sm font-semibold">{t(`kids.everywhere.accessibility.support.${key}.title`)}</p>
                  <p className="text-sm text-muted-foreground">{t(`kids.everywhere.accessibility.support.${key}.desc`)}</p>
                </div>
              </li>
            ))}
          </ul>
      ...
      <p className="mt-6 text-center text-sm text-muted-foreground">{t("kids.everywhere.accessibility.footerNote")}</p>
    </div>
  );
}
