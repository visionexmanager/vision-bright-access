...
import { useGroupBySlug } from "@/features/visionkids/hooks/social/useGroups";
import { useQuizByGroup, useSubmitQuizAttempt } from "@/features/visionkids/hooks/stories/useStoryQuiz";
...
export default function ClubQuizPage() {
  ...
  const { data: group } = useGroupBySlug(slug);
  const { data: quiz, isLoading } = useQuizByGroup(group?.id);
  ...
  useDocumentHead({ title: quiz ? `${quiz.title} — VisionKids` : t("kids.social.meta.title"), description: "", canonicalPath: `/kids/social/clubs/detail/${slug}/quiz` });
  ...
  const handleComplete = async (r: { score: number; total: number; answers: QuizAnswer[] }) => {
    ...
    if (r.score === r.total) awardAchievement.mutate("club_joiner");
    awardXp.mutate({ amount: Math.min(25, Math.round((r.score / r.total) * 25)), reason: `Group quiz completed: ${quiz.id}` });
    awardCoins.mutate({ amount: Math.min(15, Math.round((r.score / r.total) * 15)), reason: `Group quiz completed: ${quiz.id}` });
  };
  ...
  if (!quiz || !group) {
    return (
      <div className="mx-auto max-w-xl px-4 py-16 text-center">
        ...
        <Link to={`/kids/social/clubs/detail/${slug}`} className="mt-4 inline-block text-kids-primary hover:underline">{t("kids.section.backHome")}</Link>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-xl px-4 py-10 sm:px-6">
      <Link to={`/kids/social/clubs/detail/${slug}`} className="mb-4 inline-flex items-center gap-1 text-sm text-muted-foreground hover:underline">
        <ChevronLeft className="h-4 w-4" aria-hidden="true" /> {group.name}
      </Link>
        ...
        <motion.div initial="hidden" animate="visible" variants={bounceIn(reduced)} className="flex flex-col items-center gap-4 rounded-2xl border-2 border-kids-accent/40 bg-kids-accent/10 p-8 text-center">
          ...
          <div className="flex gap-2">
            ...
            <Button asChild className="bg-kids-primary text-white hover:bg-kids-primary/90">
              <Link to={`/kids/social/clubs/detail/${slug}`}>{t("kids.social.clubs.backToClub")}</Link>
            </Button>
          </div>
        </motion.div>
      ) : (
