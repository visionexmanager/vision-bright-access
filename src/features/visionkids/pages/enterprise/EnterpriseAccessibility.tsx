...
import { Sun, Moon, Eye, Type, Wand2, Keyboard, ScanEye, Captions, Volume2 } from "lucide-react";
...
export default function EnterpriseAccessibility() {
  const { t } = useLanguage();
  ...
  useDocumentHead({
    title: `${t("kids.enterprise.nav.accessibility")} — VisionKids`,
    description: t("kids.enterprise.accessibility.subtitle"),
    canonicalPath: "/kids/enterprise/accessibility",
  });
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
      <EnterpriseHeader emoji="♿" title={t("kids.enterprise.nav.accessibility")} subtitle={t("kids.enterprise.accessibility.subtitle")} showSwitcher={false} />

      <div className="mt-4 rounded-2xl border-2 border-kids-primary/30 bg-kids-primary/5 p-4">
        <p className="text-sm font-semibold">🏅 {t("kids.enterprise.accessibility.wcag")}</p>
      </div>
      ...
      <div className="mt-5 flex flex-col gap-5">
        ...
        <section className="rounded-2xl border-2 border-border bg-card p-5">
          ...
          <div className="mt-3 flex items-center justify-between gap-4">
            <Label htmlFor="ent-reduce-motion" className="text-sm font-medium">{t("kids.settings.reduceMotion")}</Label>
            <Switch id="ent-reduce-motion" checked={reduceMotion} onCheckedChange={(c) => { setKidsReduceMotion(c); bump(); }} />
          </div>
        </section>

        <section className="rounded-2xl border-2 border-border bg-card p-5">
          <h2 className="font-heading text-lg font-bold">{t("kids.enterprise.accessibility.supportTitle")}</h2>
          <ul className="mt-3 flex flex-col gap-3">
            {support.map(({ icon: Icon, key }) => (
              <li key={key} className="flex items-start gap-3">
                <Icon className="mt-0.5 h-5 w-5 shrink-0 text-kids-primary" aria-hidden="true" />
                <div>
                  <p className="text-sm font-semibold">{t(`kids.enterprise.accessibility.support.${key}.title`)}</p>
                  <p className="text-sm text-muted-foreground">{t(`kids.enterprise.accessibility.support.${key}.desc`)}</p>
                </div>
              </li>
            ))}
          </ul>
      ...
      <p className="mt-6 text-center text-sm text-muted-foreground">{t("kids.enterprise.accessibility.footerNote")}</p>
    </div>
  );
}
