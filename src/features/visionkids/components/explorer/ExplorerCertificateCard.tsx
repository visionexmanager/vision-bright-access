...
import type { ExplorerCertificate } from "@/features/visionkids/services/explorer/passport";

/** Same signing/verification scheme as Academy's CertificateCard — reuses
 *  the SAME generic verify page (/kids/academy/certificates/verify/:number),
 *  since verify_kids_certificate() and kids_certificates aren't Academy-
 *  specific, just historically routed there first. */
...
export function ExplorerCertificateCard({ certificate }: { certificate: ExplorerCertificate }) {
  const { t } = useLanguage();
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      await navigator.share({ title: certificate.title, text: t("kids.explorer.certificateShareText"), url }).catch(() => {});
    } else {
  ...
  const handleDownloadPdf = () => {
    ...
    doc.setFontSize(28);
    doc.text("VisionKids Explorer — Certificate of Achievement", pageWidth / 2, 110, { align: "center", maxWidth: pageWidth - 160 });
    ...
    doc.setFont("times", "normal");
    doc.setFontSize(14);
    doc.text("has explored every world in", pageWidth / 2, 225, { align: "center" });
    ...
    doc.save(`vision-kids-explorer-certificate-${certificate.certificate_number}.pdf`);
  };
