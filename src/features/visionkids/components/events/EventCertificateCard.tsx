...
/** Same signing/verification scheme as Academy's and Explorer's
 *  certificate cards — reuses the same generic verify page. */
...
export function EventCertificateCard({ certificate }: { certificate: KidsEventCertificate }) {
  const { t } = useLanguage();
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      await navigator.share({ title: certificate.title, text: t("kids.events.certificateShareText"), url }).catch(() => {});
    } else {
  ...
  const handleDownloadPdf = () => {
    ...
    doc.setFontSize(26);
    doc.text(certificate.title, pageWidth / 2, 110, { align: "center", maxWidth: pageWidth - 160 });

    doc.setFont("times", "normal");
    doc.setFontSize(14);
    doc.text("Awarded to", pageWidth / 2, 160, { align: "center" });
    ...
    doc.save(`vision-kids-event-certificate-${certificate.certificate_number}.pdf`);
  };
