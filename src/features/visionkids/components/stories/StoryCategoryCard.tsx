...
import { getStoryCategoryIcon, isKidsColor } from "@/features/visionkids/data/storyCategoryIcons";
import { useLanguage } from "@/contexts/LanguageContext";
import type { StoryCategory } from "@/features/visionkids/types/stories.types";
...
export function StoryCategoryCard({ category }: { category: StoryCategory }) {
  const { t } = useLanguage();
  const reduced = useKidsReducedMotion();
  const Icon = getStoryCategoryIcon(category.icon);
  ...
  return (
    <motion.div whileHover={cardHover(reduced)} whileTap={cardTap(reduced)}>
      <Link
        to={`/kids/stories/category/${category.slug}`}
        ...
        aria-label={`${category.name} — ${category.story_count} ${t("kids.stories.storiesCount")}`}
      >
        ...
        <span className="text-xs text-muted-foreground">{category.story_count} {t("kids.stories.storiesCount")}</span>
      </Link>
    </motion.div>
  );
}
