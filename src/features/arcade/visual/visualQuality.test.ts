...
describe("Arcade premium visual registry", () => {
  ...
  it("ships dedicated Full-HD raster covers for production design pack 12", () => {
    for (const slug of ["dream-home", "fashion-designer"]) {
      const game = ARCADE_GAMES.find((item) => item.slug === slug)!;
      expect(game.image).toMatch(new RegExp(`game-${slug}-premium-v2\\.webp$`));
      expect(visualsForGame(game)[0].sources[0]).toMatchObject({ width: 1920, height: 1080, mimeType: "image/webp" });
    }
  });
  it("ships dedicated Full-HD raster covers for production tabletop pack 13", () => {
    for (const slug of ["uno-ultra", "dominoes"]) {
      const game = ARCADE_GAMES.find((item) => item.slug === slug)!;
      expect(game.image).toMatch(new RegExp(`game-${slug}-premium-v2\\.webp$`));
      expect(visualsForGame(game)[0].sources[0]).toMatchObject({ width: 1920, height: 1080, mimeType: "image/webp" });
    }
  ...
  it("ships dedicated Full-HD raster covers for production classics pack 14", () => {
    for (const slug of ["farkle", "briscola", "card-99"]) {
      const game = ARCADE_GAMES.find((item) => item.slug === slug)!;
      expect(game.image).toMatch(new RegExp(`game-${slug}-premium-v2\\.webp$`));
      expect(visualsForGame(game)[0].sources[0]).toMatchObject({ width: 1920, height: 1080, mimeType: "image/webp" });
    }
  ...
  it("ships dedicated Full-HD raster covers for production finale pack 15", () => {
    for (const slug of ["logiquest", "trade-tycoon", "laptop-tech", "music-ear"]) {
      const game = ARCADE_GAMES.find((item) => item.slug === slug)!;
      expect(game.image).toMatch(new RegExp(`game-${slug}-premium-v2\\.webp$`));
      expect(visualsForGame(game)[0].sources[0]).toMatchObject({ width: 1920, height: 1080, mimeType: "image/webp" });
    }
