...
Four path and strategy routes were added: Tower Defense, Maze Runner, Parking Challenge, and Escape Room. Their tested engine provides bounded grid movement, wall collision, range-based tower damage, parking-target validation, and normalized clue answers. Grid coordinates, obstacles, targets, wave state, and clue feedback are exposed textually and through keyboard-operable controls. The playable catalog now contains 108 games. The complete suite passes 272 tests across 49 test files, and the production build succeeds after transforming 6,718 modules with all four routes code-split.
...
The heritage board collection was completed with the existing production Ludo game plus three new games: Hex, Mancala, and Royal Game of Ur. A tested shared engine implements Hex connectivity over six-direction adjacency, immutable Mancala sowing and captures, and exact-roll race movement with safe squares and captures. Each new public route is independently lazy-loaded while sharing the maintainable runtime chunk, and every board position or piece exposes a complete keyboard-readable label. Together with the five games merged into `main` immediately before this batch, the playable catalog now contains 116 games. The complete suite passes 342 tests across 59 test files, TypeScript and targeted lint checks pass, and the production build succeeds after transforming 6,766 modules with the three new routes emitted in a shared code-split chunk. Dedicated visual masters and historically mastered sound packs remain future differentiation work; this batch reuses approved project-owned Arcade visual and audio families and introduces no placeholder assets.
