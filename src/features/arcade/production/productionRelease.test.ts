...
describe("Arcade enterprise release contracts",()=>{
  ...
  it("keeps recommendations bounded and explainable",()=>{
    ...
    // The reason must resolve to real copy, not echo its own key: t() falls back
    // to the key, so a missing translation would ship "games.arcade.ai.reason.new".
    recommendations.forEach(item=>{expect(en[item.reasonKey]?.trim()).toBeTruthy();expect(item.confidence).toBeGreaterThanOrEqual(40);expect(item.confidence).toBeLessThanOrEqual(95);});
  });
