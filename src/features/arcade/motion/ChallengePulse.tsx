...
export function ChallengePulse({ progress, urgent = false }: { progress: number; urgent?: boolean }) {
  ...
  return (
    <div className="relative h-3 overflow-hidden rounded-full bg-muted" aria-hidden="true">
      <motion.div className={`absolute inset-y-0 start-0 rounded-full ${urgent ? "bg-amber-500" : "bg-gradient-to-r from-cyan-500 via-violet-500 to-primary"}`} initial={false} animate={{ width: `${safeProgress}%` }} transition={reducedMotion ? { duration: 0 } : { type: "spring", stiffness: 130, damping: 22 }} />
